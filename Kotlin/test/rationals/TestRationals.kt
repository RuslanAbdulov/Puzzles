...
class TestRationals {
    ...
    private fun testNormalizedForm(rational: String, normalizedForm: String) {
        assertEquals(normalizedForm, rational.toRational().toString(), "Wrong normalized form for $rational")
    }
    ...
    private fun testArithmeticOperation(first: String, operationRepresentation: String, second: String,
        ...
        assertEquals(expectedResult, first.toRational().operation(second.toRational()).toString(),
    ...
    private fun testComparison(
        ...
        assertEquals(expectedResult, result, "Wrong result for $first ${comparison.symbols} $second")
    }

    private fun testInRange(element: String, start: String, end: String, expected: Boolean = true) {
        assertEquals(expected, element.toRational() in start.toRational()..end.toRational(),
    ...
    fun test0Sample1Sum() {
        ...
        assertEquals( 5 divBy 6, sum, "Wrong result for sum")
    }

    @Test
    fun test0Sample2Difference() {
        val difference: Rational = (1 divBy 2) - (1 divBy 3)
        assertEquals( 1 divBy 6, difference, "Wrong result for difference")
    }

    @Test
    fun test0Sample3Product() {
        val product: Rational = (1 divBy 2) * (1 divBy 3)
        assertEquals( 1 divBy 6, product, "Wrong result for product")
    }

    @Test
    fun test0Sample4Quotient() {
        val quotient: Rational = (1 divBy 2) / (1 divBy 3)
        assertEquals(3 divBy 2, quotient, "Wrong result for quotient")
    }

    @Test
    fun test0Sample5Negation() {
        val negation: Rational = -(1 divBy 2)
        assertEquals(-1 divBy 2, negation, "Wrong result for negation")
    }

    @Test
    fun test0Sample6Integer() {
        assertEquals("2", (2 divBy 1).toString(), "Wrong string representation for integer number")
    }

    @Test
    fun test0Sample7NormalizedForm() {
        assertEquals("-1/2", (-2 divBy 4).toString(), "Wrong normalized form for '-2 divBy 4'")
        assertEquals("13/122", "117/1098".toRational().toString(), "Wrong normalized form for '117/1098'")
    }

    @Test
    fun test0Sample8Comparison() {
        assertTrue((1 divBy 2) < (2 divBy 3), "Wrong result for comparison")
    }

    @Test
    fun test0Sample9InRange() {
        assertTrue((1 divBy 2) in (1 divBy 3)..(2 divBy 3), "Wrong result for checking belonging to a range")
    }

    @Test
    fun test0Sample10Long() {
        assertEquals(1 divBy 2, 2000000000L divBy 4000000000L,
                "Wrong result for normalization of '2000000000L divBy 4000000000L'")
    }

    @Test
    fun test0Sample11BigInteger() {
        assertEquals(1 divBy 2,
